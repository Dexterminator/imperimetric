...
(deftest metric->us
  (is (= (convert-text metric-text :metric :us)
         (str "8.5 pints sugar, 0.4 cups lime, 1.7 fl. oz salt, 5.1 tsp pepper, 5.6 miles away,"
...
(deftest metric->imperial
  (is (= (convert-text metric-text :metric :imperial)
         (str "7 pints sugar, 0.3 cups lime, 1.8 fl. oz salt, 5 tsp pepper, 5.6 miles"
  ...
  (is (= (handler (request :get "/convert" {"text" "1 fl. oz of water."
         ...
         {:status  200
          :headers {"Content-Type" "application/json;charset=UTF-8"
                    "Vary"         "Accept"}
          :body    "{\"converted-text\":\"3 cl of water.\",\"original-text\":\"1 fl. oz of water.\"}"})))
